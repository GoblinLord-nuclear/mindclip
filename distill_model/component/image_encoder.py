...
class ImageEncoder(nn.Cell):
    ...
    def initialize_parameters(self):
        normal_initializer_cls = Normal(sigma=0.02)
        normal_initializer_pos = Normal(sigma=0.01)
        self.visual.class_embedding.weight.set_data(normal_initializer_cls(self.visual.class_embedding.weight.shape))
        self.visual.positional_embedding.weight.set_data(normal_initializer_pos(self.visual.positional_embedding.weight.shape))
        ...
        normal_initializer_attn = Normal(sigma=attn_std)
        normal_initializer_proj = Normal(sigma=proj_std)
        normal_initializer_fc = Normal(sigma=fc_std)
        for block in self.visual.transformer.resblocks:
            block.attn.in_proj_weight.set_data(normal_initializer_attn(block.attn.in_proj_weight.shape))
            block.attn.in_proj_bias.set_data(normal_initializer_attn(block.attn.in_proj_bias.shape))
            block.attn.out_proj.weight.set_data(normal_initializer_proj(block.attn.out_proj.weight.shape))
            block.mlp.c_fc.weight.set_data(normal_initializer_fc(block.mlp.c_fc.weight.shape))
            block.mlp.c_proj.weight.set_data(normal_initializer_proj(block.mlp.c_proj.weight.shape))
